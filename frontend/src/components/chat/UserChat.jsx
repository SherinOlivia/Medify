...
const UserChat = ({ chat, data }) => {
    const { recipientUser } = useFetchRecipientUser(chat, data);
    const { onlineUsers, notifications } = useContext(ChatContext);
    const { latestMessage } = useFetchLatestMessage(chat);
    
    const unreadNotifications = unreadNotificationsFunc(notifications)
    const thisUserNotifications = unreadNotifications?.filter
    recipientUser?.data?.username
    ...
    const isOnline = onlineUsers?.some((data) => data?.userId === recipientUser?.data?._id)
